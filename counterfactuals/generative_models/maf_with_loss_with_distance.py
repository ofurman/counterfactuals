...
class MaskedAutoregressiveFlowDistance(Flow, BaseGenModel):
    def __init__(
        self,
        features,
        hidden_features,
    ...
    def forward(self, x, context=None):
        if context is not None:
            context = context.reshape(-1, self.context_features).to(self.device)  # ✅ Fix added
    ...
    def fit(
            ...
            patience: int = 20,
            eps: float = 1e-3,
            lambda_dist: float = 0.2,  # Weight for distance penalty
            checkpoint_path: str = "best_model_with_distance.pth",
        ...
        for epoch in (pbar := tqdm(range(num_epochs))):
            ...
            for inputs, labels in train_loader:
                ...
                if labels.ndim == 1:  # If labels are 1D
                    labels = labels.view(-1, self.context_features)  # Reshape to [batch_size, context_features]
                elif labels.shape[1] != self.context_features:  # If labels are multi-dimensional but not aligned
                    raise ValueError(
                        f"Labels shape {labels.shape} is incompatible with context_features={self.context_features}"
                    )
                ...
                # Distance component
                distance = torch.norm(inputs - labels, p=2, dim=1)
                distance_loss = distance.mean()

                # Combine the losses
                loss = -log_likelihood.mean() + lambda_dist * distance_loss
    ...
    def predict_log_prob(self, dataloader) -> torch.Tensor:
        ...
        with torch.no_grad():
            for inputs, labels in dataloader:
                inputs, labels = inputs.to(self.device), labels.to(self.device)  # ✅ Move to CUDA
                outputs = self(inputs, labels)
                log_probs.append(outputs)

        results = torch.concat(log_probs).cpu()  # ✅ Move to CPU before returning
    ...
    def sample_and_log_prob(self, num_samples, context=None, temp=1.0):
        ...
        embedded_context = self._embedding_net(context).to(self.device)  # ✅ Ensure CUDA
        ...
        samples, logabsdet = self._transform.inverse(noise.to(self.device), context=embedded_context)
    ...
    def load(self, path):
        self.load_state_dict(torch.load(path, map_location=self.device))  # ✅ Fix added
