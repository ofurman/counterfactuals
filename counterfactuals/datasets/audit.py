...
class AuditDataset(AbstractDataset):
    def __init__(self, file_path: str = "data/audit.csv", transform=True, shuffle=True):
        """
        Initialize the Audit dataset.
        """
    ...
    def preprocess(self, raw_data: pd.DataFrame):
        ...
        self.target_column = raw_data.columns[-1]
        self.feature_columns = list(raw_data.columns[2:-1])
        self.feature_columns.remove("Detection_Risk")
        self.features = self.feature_columns + [self.target_column]
        ...
        row_per_class = sum(raw_data[self.target_column] == 1)
        raw_data = pd.concat(
            [
                raw_data[raw_data[self.target_column] == 0].sample(
                    row_per_class, random_state=42
                ),
                raw_data[raw_data[self.target_column] == 1],
            ]
        )
