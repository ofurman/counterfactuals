...
class DiabetesDataset(AbstractDataset):
    def __init__(self, file_path: str = "data/regression/diabetes.csv"):
    ...
    def transform(
        ...
        y_train = self.label_transformer.fit_transform(y_train.reshape(-1, 1)).reshape(
            -1
        )
        y_test = self.label_transformer.transform(y_test.reshape(-1, 1)).reshape(-1)
    ...
    def train_dataloader(
        ...
        **kwargs_dataloader,
        ...
        def collate_fn(batch):
            ...
            noise = torch.randn_like(y) * label_noise_lvl
            y = y + noise
            return X, y
