...
class CET(BaseCounterfactual):
    ...
    def explain_dataloader(
        ...
        #     try:
        #         expl = dice_expl.compute_counterfactual(X_instances[i, :], 1)[0].flatten()
        #         X_cfs.append(expl)
        #         X_idx.append(i)
        #     except Exception as e:
        #         pass

        # X_instances = np.array(X_instances[X_idx,:])  # Remove instances for which no counterfactual was found!
        # X_cfs = np.array(X_cfs)

        # Cluster instances
        # ["dbscan-cf", "dbscan-xorig"]
        # clustering = cluster_instances(X_instances, X_cfs, method="dbscan-xorig").labels_
        # print(f"Clustering: {clustering}")

        # Compute multi-instance counterfactuals
        def compute_multiinstance_cf(X_inst: np.ndarray, multiinst_method="kanamori"):
            clf = self.disc_model
            y_target = 1
            ...
            if multiinst_method == "kanamori":
                expl = GroupCF(
                    clf, self.features_desc, X_train.values, y_train.values, 1
                )
        ...
        # GLOBAL
        cf_deltas = compute_multiinstance_cf(Xs)
        print("CF Deltas")
        print(cf_deltas)
        Xs_cfs = Xs + cf_deltas
        print("Xs_cfs")
        ...
        # # GROUP-WISE
        # Xs_cfs = []
        # for l in tqdm(np.unique(clustering)): # Local
        #     idx = clustering == l
