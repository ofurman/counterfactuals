...
class GenCECond(BaseGenModel):
    ...
    def proximity(self, x_cf, x_orig):
    ...
    def fit(
        ...
        for epoch in (pbar := tqdm(range(num_epochs))):
            ...
            for x_cf, x_orig in train_loader:
                ...
                optimizer.zero_grad()
                context = torch.hstack([x_orig, self.proximity(x_cf, x_orig)])
                log_likelihood = self(
                    x_cf, torch.hstack([x_orig, self.proximity(x_cf, x_orig)])
                )
