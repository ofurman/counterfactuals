...
class FiLM(nn.Module):
    ...
    def forward(self, h: torch.Tensor, c: torch.Tensor) -> torch.Tensor:
        ...
        gamma = 1.0 + torch.tanh(gamma) * 0.1 # start close to 1
        beta = torch.tanh(beta) * 0.1         # start close to 0
        return h * gamma + beta
...
class TabularEpsModel(nn.Module):
    def __init__(self, xdim: int, cat_dims: list[int], y_classes: int, 
                 hidden: int = 256, nblocks: int = 4, tdim: int = 64, dropout: float = 0.1):
        super().__init__()
    ...
    def forward(self, x_t: torch.Tensor, t: torch.Tensor, x_orig: torch.Tensor, y_target: torch.Tensor) -> torch.Tensor:
