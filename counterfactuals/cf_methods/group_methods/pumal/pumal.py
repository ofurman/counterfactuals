...
from counterfactuals.cf_methods.group_methods.pumal.deltas import (
    ARES,
    GCE,
    GLOBAL_CE,
    PPCEF_2,
...
class PUMAL(BaseCounterfactualMethod):
    def __init__(
        self,
        cf_method_type: str,
    ...
    def explain_dataloader(
        ...
        for xs_origin, contexts_origin in dataloader:
            ...
            num_classes = self.disc_model.num_targets
            if num_classes is None:
                num_classes = contexts_origin.shape[1]
            contexts_target = torch.zeros(
                xs_origin.shape[0],
                num_classes,
                device=xs_origin.device,
                dtype=contexts_origin.dtype,
            )
