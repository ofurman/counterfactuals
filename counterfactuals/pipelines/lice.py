...
from counterfactuals.cf_methods.local_methods.lice.nn_model import NNModel
...
def main(argv: list[str] | None = None) -> int:
    ...
    if len(args) < 5:
        raise ValueError("Usage: lice.py <time_limit> <data_name> <fold> <mode> <folder> [alpha]")

    time_limit = int(args[0])
    data_names = [args[1]]
    folds = [int(args[2])]
    median = args[3] == "median"
    quartile = args[3] == "quartile"
    optimize = args[3] == "optimize"
    folder = args[4]
    alpha = 0.1 if len(args) <= 5 else float(args[5])
