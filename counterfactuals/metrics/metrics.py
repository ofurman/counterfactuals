...
def number_valid_cf(cf_list, b, y_val, y_desidered=None):
...
def perc_valid_cf(cf_list, b, y_val, k=None, y_desidered=None):
    n_val = number_valid_cf(cf_list, b, y_val, y_desidered)
    k = len(cf_list) if k is None else k
    res = n_val / k
    return res


def number_actionable_cf(x, cf_list, variable_features):
    number_actionable = 0
...
def perc_actionable_cf(x, cf_list, variable_features, k=None):
    n_val = number_actionable_cf(x, cf_list, variable_features)
    k = len(cf_list) if k is None else k
    res = n_val / k
    return res


def number_valid_actionable_cf(x, cf_list, b, y_val, variable_features, y_desidered=None):
    y_cf = b.predict(cf_list)
    idx = y_cf != y_val if y_desidered is None else y_cf == y_desidered

    number_valid_actionable = 0
    number_features = cf_list.shape[1]
    for i, cf in enumerate(cf_list):
        ...
        for j in range(number_features):
            if cf[j] != x[j] and j not in variable_features:
                ...
                break
        if not constraint_violated:
            number_valid_actionable += 1

    return number_valid_actionable


def perc_valid_actionable_cf(x, cf_list, b, y_val, variable_features, k=None, y_desidered=None):
    n_val = number_valid_actionable_cf(x, cf_list, b, y_val, variable_features, y_desidered)
    k = len(cf_list) if k is None else k
    res = n_val / k
    return res


def number_violations_per_cf(x, cf_list, variable_features):
    number_features = cf_list.shape[1]
    number_violations = np.zeros(len(cf_list))
    for i, cf in enumerate(cf_list):
        for j in range(number_features):
            if cf[j] != x[j] and j not in variable_features:
                number_violations[i] += 1
    return number_violations


def avg_number_violations_per_cf(x, cf_list, variable_features):
    return np.mean(number_violations_per_cf(x, cf_list, variable_features))


def avg_number_violations(x, cf_list, variable_features):
    val = np.sum(number_violations_per_cf(x, cf_list, variable_features))
    number_cf, number_features = cf_list.shape
...
def number_changes_per_cf(x, cf_list, continuous_features):
    number_features = cf_list.shape[1]
    number_changes = np.zeros(len(cf_list))
    for i, cf in enumerate(cf_list):
        for j in range(number_features):
            if cf[j] != x[j]:
                number_changes[i] += 1 if j in continuous_features else 0.5
    return number_changes


def avg_number_changes_per_cf(x, cf_list, continuous_features):
    return np.mean(number_changes_per_cf(x, cf_list, continuous_features))


def avg_number_changes(x, cf_list, number_features, continuous_features):
    val = np.sum(number_changes_per_cf(x, cf_list, continuous_features))
    number_cf, _ = cf_list.shape
    return val / (number_cf * number_features)
...
def plausibility(x, classifier, cf_list, X_test, y_pred, continuous_features_all,
