...
def main(cfg: DictConfig):
    ...
    for fold_n, (_, _, _, _) in enumerate(dataset.get_cv_splits(n_splits=5)):
        ...
        if cfg.experiment.relabel_with_disc_model:
            dataset.y_train = disc_model.predict(dataset.X_train).detach().cpu().numpy()
            dataset.y_test = disc_model.predict(dataset.X_test).detach().cpu().numpy()
        ...
        pd.DataFrame({"train_ll": [train_ll], "test_ll": [test_ll]}).to_csv(
            os.path.join(
                save_folder,
                f"eval_gen_model_{fold_n}_{gen_model_name}_relabeled_by_{disc_model_name}.csv",
            )
        )
