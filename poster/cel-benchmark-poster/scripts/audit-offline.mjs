...
const result = await withPosterPage(async ({ page, failures, remoteRequests, consoleErrors }) => {
  const rendered = await page.evaluate(() => ({
    ...
    qrLogoSource: document.querySelector('.project-mark__qr image')?.getAttribute('href') ?? '',
  ...
  if (!rendered.qrLogoSource.startsWith('data:image/svg+xml')) defects.push('The QR GitHub logo must be embedded as an inline SVG for offline use')
