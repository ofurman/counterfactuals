...
if (/<script\b[^>]*\bsrc\s*=|<link\b[^>]*rel=["']?stylesheet|<img\b[^>]*\bsrc\s*=/i.test(bundle)) throw new Error('Bundle contains a required external or local asset reference')
...
const result = await withPosterPage(async ({ page, failures, remoteRequests, consoleErrors }) => {
  ...
  if (!rendered.canvas || rendered.sections !== 10 || rendered.claimMarkers !== 42 || rendered.readyState !== 'complete') defects.push(`Incomplete offline render: ${JSON.stringify(rendered)}`)
  if (defects.length) throw new Error(defects.join('\n'))
  return rendered
}, { entry: 'bundle', blockRemote: true })

console.log(`Offline bundle audit passed: bytes=${bundleStat.size}, sections=${result.sections}, claims=${result.claimMarkers}, required remote requests=0`)
