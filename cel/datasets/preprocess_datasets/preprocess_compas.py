...
def preprocess_compas(raw_data: pd.DataFrame) -> pd.DataFrame:
    ...
    raw_data["days_b_screening_arrest"] = np.abs(raw_data["days_b_screening_arrest"])
    raw_data["c_jail_out"] = pd.to_datetime(raw_data["c_jail_out"])
    raw_data["c_jail_in"] = pd.to_datetime(raw_data["c_jail_in"])
    raw_data["length_of_stay"] = np.abs((raw_data["c_jail_out"] - raw_data["c_jail_in"]).dt.days)
    ...
    raw_data["days_b_screening_arrest"] = raw_data["days_b_screening_arrest"].astype(int)
    # raw_data = raw_data[raw_data["score_text"] != "Medium"]
    # raw_data["class"] = pd.get_dummies(raw_data["score_text"])["High"].astype(int)
    raw_data["class"] = raw_data["score_text"].map({"Low": 0, "Medium": 1, "High": 2}).astype(int)
