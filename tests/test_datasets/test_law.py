...
def test_law_initialization():
    # Construct absolute path to the config file
    project_root = Path(__file__).parent.parent.parent
    config_path = project_root / "config/datasets/law.yaml"
...
def test_law_ppcef_execution():
    """
    Integration test to verify that PPCEF pipeline runs on Law dataset.
    """
        ...
        try:
            with initialize(version_base="1.2", config_path=config_dir):
                cfg = compose(
                    ...
                    overrides=[
                        "dataset.config_path=config/datasets/law.yaml",
...
def test_law_globe_ce_execution():
    """
    Integration test to verify that GLOBE-CE pipeline runs on Law dataset.
    """
    with tempfile.TemporaryDirectory() as tmp_dir:
        GlobalHydra.instance().clear()
        config_dir = "../../counterfactuals/pipelines/conf"
