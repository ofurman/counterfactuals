...
def test_lending_club_initialization():
    # Construct absolute path to the config file
    project_root = Path(__file__).parent.parent.parent
    config_path = project_root / "config/datasets/lending_club.yaml"
...
def test_lending_club_ppcef_execution():
    """
    Integration test to verify that PPCEF pipeline runs on Lending Club dataset.
    """
        ...
        try:
            with initialize(version_base="1.2", config_path=config_dir):
                cfg = compose(
                    ...
                    overrides=[
                        "dataset.config_path=config/datasets/lending_club.yaml",
...
def test_lending_club_globe_ce_execution():
    """
    Integration test to verify that GLOBE-CE pipeline runs on Lending Club dataset.
    """
    with tempfile.TemporaryDirectory() as tmp_dir:
        GlobalHydra.instance().clear()
        config_dir = "../../counterfactuals/pipelines/conf"
