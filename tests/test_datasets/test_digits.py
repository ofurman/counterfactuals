...
def test_digits_initialization():
    # Construct absolute path to the config file
    project_root = Path(__file__).parent.parent.parent
    config_path = project_root / "config/datasets/digits.yaml"
    ...
    # Verify features are loaded (64 pixels for 8x8 images)
    assert len(dataset.features) == 64
    assert dataset.X.shape[1] == 64


def test_digits_ppcef_execution():
    """
    Integration test to verify that PPCEF pipeline runs on Digits dataset.
    """
        ...
        try:
            with initialize(version_base="1.2", config_path=config_dir):
                cfg = compose(
                    ...
                    overrides=[
                        "dataset.config_path=config/datasets/digits.yaml",
                        "++dataset.samples_keep=500",  # Smaller for faster testing
...
def test_digits_globe_ce_execution():
    """
    Integration test to verify that GLOBE-CE pipeline runs on Digits dataset.
    """
    with tempfile.TemporaryDirectory() as tmp_dir:
        GlobalHydra.instance().clear()
        config_dir = "../../counterfactuals/pipelines/conf"
