...
def test_moons_initialization():
    # Construct absolute path to the config file
    project_root = Path(__file__).parent.parent.parent
    config_path = project_root / "config/datasets/moons.yaml"
    ...
    # Verify features are loaded (2 features for moons)
    assert len(dataset.features) > 0
    assert dataset.X.shape[1] == len(dataset.features)


def test_moons_ppcef_execution():
    """
    Integration test to verify that PPCEF pipeline runs on Moons dataset.
    """
        ...
        try:
            with initialize(version_base="1.2", config_path=config_dir):
                cfg = compose(
                    ...
                    overrides=[
                        "dataset.config_path=config/datasets/moons.yaml",
...
def test_moons_globe_ce_execution():
    """
    Integration test to verify that GLOBE-CE pipeline runs on Moons dataset.
    """
    with tempfile.TemporaryDirectory() as tmp_dir:
        GlobalHydra.instance().clear()
        config_dir = "../../counterfactuals/pipelines/conf"
