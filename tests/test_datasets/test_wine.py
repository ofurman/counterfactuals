...
def test_wine_initialization():
    ...
    config_path = project_root / "config/datasets/wine.yaml"
    ...
    # Verify features are loaded (13 features for wine)
    assert len(dataset.features) > 0
    assert dataset.X.shape[1] == len(dataset.features)


def test_wine_ppcef_execution():
    """
    Integration test to verify that PPCEF pipeline runs on Wine dataset.
    """
        ...
        try:
            with initialize(version_base="1.2", config_path=config_dir):
                cfg = compose(
                    ...
                    overrides=[
                        "dataset.config_path=config/datasets/wine.yaml",
                        ...
                        "counterfactuals_params.epochs=10",
...
def test_wine_globe_ce_execution():
    """
    Integration test to verify that GLOBE-CE pipeline runs on Wine dataset.
    """
    with tempfile.TemporaryDirectory() as tmp_dir:
        GlobalHydra.instance().clear()
        config_dir = "../../counterfactuals/pipelines/conf"
