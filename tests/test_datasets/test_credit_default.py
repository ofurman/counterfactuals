...
def test_credit_default_initialization():
    # Construct absolute path to the config file
    project_root = Path(__file__).parent.parent.parent
    config_path = project_root / "config/datasets/credit_default.yaml"
...
def test_credit_default_ppcef_execution():
    """
    Integration test to verify that PPCEF pipeline runs on Credit Default dataset.
    """
        ...
        try:
            with initialize(version_base="1.2", config_path=config_dir):
                cfg = compose(
                    ...
                    overrides=[
                        "dataset.config_path=config/datasets/credit_default.yaml",
...
def test_credit_default_globe_ce_execution():
    """
    Integration test to verify that GLOBE-CE pipeline runs on Credit Default dataset.
    """
    # Create a temporary directory for output
    with tempfile.TemporaryDirectory() as tmp_dir:
        # Clear any existing Hydra instance
