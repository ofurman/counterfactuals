...
def test_give_me_some_credit_initialization():
    # Construct absolute path to the config file
    project_root = Path(__file__).parent.parent.parent
    config_path = project_root / "config/datasets/give_me_some_credit.yaml"
...
def test_give_me_some_credit_ppcef_execution():
    """
    Integration test to verify that PPCEF pipeline runs on Give Me Some Credit dataset.
    """
        ...
        try:
            with initialize(version_base="1.2", config_path=config_dir):
                cfg = compose(
                    ...
                    overrides=[
                        "dataset.config_path=config/datasets/give_me_some_credit.yaml",
...
def test_give_me_some_credit_globe_ce_execution():
    """
    Integration test to verify that GLOBE-CE pipeline runs on Give Me Some Credit dataset.
    """
    with tempfile.TemporaryDirectory() as tmp_dir:
        GlobalHydra.instance().clear()
        config_dir = "../../counterfactuals/pipelines/conf"
