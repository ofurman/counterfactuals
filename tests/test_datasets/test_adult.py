...
def test_adult_initialization():
    # Construct absolute path to the config file
    project_root = Path(__file__).parent.parent.parent
    config_path = project_root / "config/datasets/adult.yaml"
...
def test_adult_ppcef_execution():
    """
    Integration test to verify that PPCEF pipeline runs on Adult dataset.
...
def test_adult_globe_ce_execution():
    """
    Integration test to verify that GLOBE-CE pipeline runs on Adult dataset.
    This runs for 1 epoch on a tiny subset to ensure end-to-end execution works.
    """
    # Create a temporary directory for output
    with tempfile.TemporaryDirectory() as tmp_dir:
