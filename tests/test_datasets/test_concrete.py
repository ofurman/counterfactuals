...
def test_concrete_initialization():
    ...
    # Verify features are loaded (8 features + 1 target in csv, config says 8 features)
    assert len(dataset.features) == 8
    assert dataset.X.shape[1] == 8


@pytest.mark.skip(reason="Concrete is a regression dataset, PPCEF pipeline expects classification currently.")
def test_concrete_ppcef_execution():
    """
    Integration test to verify that PPCEF pipeline runs on Concrete dataset.
    """
...
@pytest.mark.skip(reason="Concrete is a regression dataset, GLOBE-CE pipeline expects classification currently.")
def test_concrete_globe_ce_execution():
    """
    Integration test to verify that GLOBE-CE pipeline runs on Concrete dataset.
    """
    with tempfile.TemporaryDirectory() as tmp_dir:
        GlobalHydra.instance().clear()
        config_dir = "../../counterfactuals/pipelines/conf"
