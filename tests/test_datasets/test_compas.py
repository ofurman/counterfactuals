...
def test_compas_initialization():
    # Construct absolute path to the config file
    project_root = Path(__file__).parent.parent.parent
    config_path = project_root / "config/datasets/compas.yaml"
...
def test_compas_ppcef_execution():
    """
    Integration test to verify that PPCEF pipeline runs on Compas dataset.
    """
...
def test_compas_globe_ce_execution():
    """
    Integration test to verify that GLOBE-CE pipeline runs on Compas dataset.
    """
    with tempfile.TemporaryDirectory() as tmp_dir:
        GlobalHydra.instance().clear()
        config_dir = "../../counterfactuals/pipelines/conf"
            ...
            with initialize(version_base="1.2", config_path=config_dir):
                cfg = compose(
                    ...
                    overrides=[
                        ...
                        "counterfactuals_params.batch_size=16",
                        "counterfactuals_params.target_class=1", # different target
                        f"experiment.output_folder={tmp_dir}",
                    ]
                )
        except ValueError as e:
