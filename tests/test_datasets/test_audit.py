...
def test_audit_initialization():
    ...
    config_path = project_root / "config/datasets/audit.yaml"
...
def test_audit_ppcef_execution():
    """
    Integration test to verify that PPCEF pipeline runs on Audit dataset.
        ...
        try:
            with initialize(version_base="1.2", config_path=config_dir):
                cfg = compose(
                    ...
                    overrides=[
                        "dataset.config_path=config/datasets/audit.yaml",
...
def test_audit_globe_ce_execution():
    """
    Integration test to verify that GLOBE-CE pipeline runs on Audit dataset.
    This runs for 1 epoch on a tiny subset to ensure end-to-end execution works.
    """
    # Create a temporary directory for output
    with tempfile.TemporaryDirectory() as tmp_dir:
