...
def test_bank_marketing_initialization():
    # Construct absolute path to the config file
    project_root = Path(__file__).parent.parent.parent
    config_path = project_root / "config/datasets/bank_marketing.yaml"
...
def test_bank_marketing_ppcef_execution():
    """
    Integration test to verify that PPCEF pipeline runs on Bank Marketing dataset.
        ...
        try:
            with initialize(version_base="1.2", config_path=config_dir):
                cfg = compose(
                    ...
                    overrides=[
                        "dataset.config_path=config/datasets/bank_marketing.yaml",
...
def test_bank_marketing_globe_ce_execution():
    """
    Integration test to verify that GLOBE-CE pipeline runs on Bank Marketing dataset.
    This runs for 1 epoch on a tiny subset to ensure end-to-end execution works.
    """
    # Create a temporary directory for output
    with tempfile.TemporaryDirectory() as tmp_dir:
