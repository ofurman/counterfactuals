...
def _mask_metrics_for_zero_validity(
    ...
    validity_value = None if validity_cell is None else _cell_numeric_value(validity_cell)
...
def build_latex_table(
    ...
    *,
    ...
    ncols = 2 + len(metric_keys)
    col_spec = "l|l|" + ("r" * len(metric_keys))
    ...
    for model in models:
        ...
        for dataset in datasets_by_model[model]:
            ...
            dataset_tex = fmt_name(dataset, dataset_aliases)
            for method_index, method in enumerate(methods):
                ...
                if method_index == 0:
                    left = rf"\multirow{{{len(methods)}}}{{*}}{{{dataset_tex}}}"
                    line = " & ".join([left, method_tex, *metric_cells]) + r" \\"
                else:
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="Comma-separated list of datasets to include (repeated allowed). If set, restricts output.",
    )
    parser.add_argument(
        "--exclude-datasets",
        action="append",
        default=[],
        help="Comma-separated list of datasets to exclude (repeated allowed).",
    )
    parser.add_argument(
        "--drop-empty-rows",
        action="store_true",
    ...
    parser.add_argument(
        "--proximity-prefix",
        default="proximity_",
        help="Prefix used to classify proximity metrics (default: proximity_).",
    )
    parser.add_argument(
        "--caption-proximity",
        default="Proximity metrics across datasets and classification models.",
        help="Caption for proximity table.",
    )
    parser.add_argument(
        "--label-proximity",
        default="tab:proximity_metrics",
        help="LaTeX label for proximity table.",
    )
    parser.add_argument(
        "--caption-other",
        default="Additional metrics across datasets and classification models.",
        help="Caption for other-metrics table.",
    )
    parser.add_argument(
        "--label-other",
        default="tab:other_metrics",
        help="LaTeX label for other-metrics table.",
    )
    parser.add_argument(
        "--combine-models",
        action="store_true",
        help=(
            "Combine all models into a single table with model sections. "
            "Default is to emit separate tables per model."
        ),
    )
    parser.add_argument(
        "--config-yaml",
...
def _label_suffix(model: str) -> str:
    safe = "".join(ch if (ch.isalnum() or ch == "_") else "_" for ch in model)
...
def main() -> None:
    ...
    include_metrics = _parse_list_args(args.include_metrics)
    ...
    include_datasets = _parse_list_args(args.include_datasets)
    exclude_datasets = set(_parse_list_args(args.exclude_datasets))
    ...
    other_metrics = [m for m in metric_keys if m not in proximity_metrics]

    if not proximity_metrics:
        logging.warning("No proximity metrics detected (prefix=%s).", args.proximity_prefix)
    if not other_metrics:
        logging.warning("No non-proximity metrics detected.")
    ...
    models = _unique_in_order([rk.model for rk in row_keys])
    datasets_by_model: dict[str, list[str]] = {m: [] for m in models}
    methods_by_model: dict[str, list[str]] = {m: [] for m in models}
    for rk in row_keys:
        if rk.dataset not in datasets_by_model[rk.model]:
            datasets_by_model[rk.model].append(rk.dataset)
        if rk.method not in methods_by_model[rk.model]:
            methods_by_model[rk.model].append(rk.method)
    ...
    # Apply dataset include/exclude filters (preserve order where possible).
    if include_datasets:
        include_datasets_ordered = _unique_in_order(include_datasets)
        for model in models:
            datasets_by_model[model] = [
                d for d in include_datasets_ordered if d in datasets_by_model[model]
            ]
    for model in models:
        datasets_by_model[model] = [
            d for d in datasets_by_model[model] if d not in exclude_datasets
        ]
    ...
    if args.combine_models:
        if proximity_metrics:
            latex_parts.append(
                build_latex_table(
                    row_keys,
                    records,
                    metric_keys=proximity_metrics,
                    metric_meta=metric_meta,
                    caption=args.caption_proximity,
                    label=args.label_proximity,
                    model=None,
        ...
        if other_metrics:
            latex_parts.append(
                build_latex_table(
                    row_keys,
                    records,
                    metric_keys=other_metrics,
                    metric_meta=metric_meta,
                    caption=args.caption_other,
                    label=args.label_other,
                    model=None,
                    model_aliases=model_aliases,
                    dataset_aliases=dataset_aliases,
                    method_aliases=method_aliases,
        ...
        for model in models:
            model_tex = model_aliases.get(model, model)
            if proximity_metrics:
                latex_parts.append(
                    build_latex_table(
                        row_keys,
                        ...
                        metric_keys=proximity_metrics,
                        metric_meta=metric_meta,
                        caption=f"{args.caption_proximity} ({model_tex})",
                        label=f"{args.label_proximity}_{_label_suffix(model)}",
                        model=model,
                        model_aliases=model_aliases,
                        dataset_aliases=dataset_aliases,
                        method_aliases=method_aliases,
            ...
            if other_metrics:
                latex_parts.append(
                    build_latex_table(
                        ...
                        metric_keys=other_metrics,
                        metric_meta=metric_meta,
                        caption=f"{args.caption_other} ({model_tex})",
                        label=f"{args.label_other}_{_label_suffix(model)}",
                        model=model,
                        model_aliases=model_aliases,
                        dataset_aliases=dataset_aliases,
                        method_aliases=method_aliases,
